...
def Greens(a, alpha, k, epsilon_s, z_min, z_max):   
    q = k * np.sqrt(epsilon_s)
    alpha = (np.sqrt(epsilon_s) + 1)/(np.sqrt(epsilon_s) - 1) 
    X1 = (np.exp(complex(0,q*(a-z_max))) + alpha*np.exp(complex(0,-q*(a-z_max)))) \
        / (np.exp(complex(0,q*a)) + alpha*np.exp(complex(0,-q*a)))
    X2 = (np.exp(complex(0,q*(a+z_min))) + alpha*np.exp(complex(0,-q*(a+z_min)))) \
        / ( alpha*np.exp(complex(0,-q*a)) - np.exp(complex(0,q*a)))
    return complex(0,-1/(2*q)) * X1 * X2
